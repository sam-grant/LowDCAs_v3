...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsReco_15921/NoQ
...
for file in `ls SplitFileList*`; do 
  ...
  if [ -f ${pnfsOutDir}/RunRecoAndPlotNoQ.fcl ]; then
    rm -f ${pnfsOutDir}/RunRecoAndPlotNoQ.fcl  
  fi
  ...
  ifdh cp RunRecoAndPlotNoQ.fcl ${pnfsOutDir}/RunRecoAndPlotNoQ.fcl
  ...
  echo "Copied RunRecoAndPlotNoQ.fcl, libgm2tracker_analyses_LowDCAsPlots_module.so, and xrootdFileList${fileNum} to ${pnfsOutDir}"
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/RunRecoAndPlotNoQ.fcl\`" ]; then
  ifdh cp ${pnfsOutDir}/RunRecoAndPlotNoQ.fcl ./RunRecoAndPlotNoQ.fcl
else 
  echo "${pnfsOutDir}/RunRecoAndPlotNoQ.fcl does not exist"
fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  gm2 -c RunRecoAndPlotNoQ.fcl -s \$file -T recoPlots\${id}.root
