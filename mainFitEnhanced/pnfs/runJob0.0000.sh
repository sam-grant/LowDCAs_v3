...
if [ ! -z "`ifdh ls /pnfs/GM2/scratch/users/sgrant/pValueScanMainFitEnhanced/0.0000.fcl`" ]; then
  ifdh cp /pnfs/GM2/scratch/users/sgrant/pValueScanMainFitEnhanced/0.0000.fcl ./0.0000.fcl
else 
  echo "/pnfs/GM2/scratch/users/sgrant/pValueScanMainFitEnhanced/0.0000.fcl does not exist"
fi
...
gm2 -c 0.0000.fcl -s root://fndca1.fnal.gov:1094/pnfs/fnal.gov/usr/GM2/daq/run1/offline/gm2_5037A/runs_15000/15921/gm2offline_full_17632340_15921.00313.root -T recoPlots0.0000.root | tee 0.0000.txt

# Only copy back if job completed successfully (output from last command was 0)
if [ $? -eq 0 ]; then
	if [ -f recoPlots0.0000.root ]; then
    	ifdh mv recoPlots0.0000.root /pnfs/GM2/scratch/users/sgrant/pValueScanMainFitEnhanced/recoPlots0.0000.root
    else 
    	echo "recoPlots0.0000.root does not exist.  ls reads:"
    	ls
    fi
    if [ -f 0.0000.txt ]; then
    	ifdh mv 0.0000.txt /pnfs/GM2/scratch/users/sgrant/pValueScanMainFitEnhanced/0.0000.txt
    else 
    	echo "0.0000.txt does not exist.  ls reads:"
    	ls
    fi
fi
