...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsRecoScan_15921/NoQ
...
for file in `ls SplitFileList*`; do 
  ...
  for line in `cat $file`; do
    ...
    if [ -f ${pnfsOutDir}/recoCoarseScanPlotsNoQ${id}.root ]; then
      echo "${pnfsOutDir}/recoCoarseScanPlotsNoQ${id}.root already exists, skipping..."
      continue
    fi
  ...
  if [ -f ${pnfsOutDir}/RunRecoAndPlotCoarseScanNoQ.fcl ]; then
    rm -f ${pnfsOutDir}/RunRecoAndPlotCoarseScanNoQ.fcl  
  fi
  ...
  ifdh cp RunRecoAndPlotCoarseScanNoQ.fcl ${pnfsOutDir}/RunRecoAndPlotCoarseScanNoQ.fcl
  ...
  echo "Copied RunRecoAndPlotCoarseScanNoQ.fcl, libgm2tracker_analyses_LowDCAsPlots_module.so, and xrootdFileList${fileNum} to ${pnfsOutDir}"


# Make script that we'll want to run on the grid
cat <<EOF > runRecoCoarseScanNoQJob${fileNum}.sh
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/RunRecoAndPlotCoarseScanNoQ.fcl\`" ]; then
  ifdh cp ${pnfsOutDir}/RunRecoAndPlotCoarseScanNoQ.fcl ./RunRecoAndPlotCoarseScanNoQ.fcl
else 
  echo "${pnfsOutDir}/RunRecoAndPlotCoarseScanNoQ.fcl does not exist"
fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  gm2 -c RunRecoAndPlotCoarseScanNoQ.fcl -s \$file -T recoCoarseScanPlotsNoQ\${id}.root

  # Only copy back if job completed successfully (output from last command was 0)
  if [ \$? -eq 0 ]; then
    if [ -f recoCoarseScanPlotsNoQ\${id}.root ]; then
      ifdh mv recoCoarseScanPlotsNoQ\${id}.root ${pnfsOutDir}/recoCoarseScanPlotsNoQ\${id}.root
    else 
      echo "recoCoarseScanPlotsNoQ\${id}.root does not exist.  ls reads:"
      ls
    fi
  fi
done
EOF
  ...
  if [ -f ${pnfsOutDir}/runRecoCoarseScanNoQJob${fileNum}.sh ]; then
    rm -f ${pnfsOutDir}/runRecoCoarseScanNoQJob${fileNum}.sh
  fi
    ifdh cp runRecoCoarseScanNoQJob${fileNum}.sh ${pnfsOutDir}/runRecoCoarseScanNoQJob${fileNum}.sh
  while [ ! -f ${pnfsOutDir}/runRecoCoarseScanNoQJob${fileNum}.sh ]; do
    echo "${pnfsOutDir}/runRecoCoarseScanNoQJob${fileNum}.sh not found after ifdh cp.  Sleeping 5..."
    sleep 5
  done

  #Submit grid job
  jobsub_submit -N 1 -G gm2 --OS=SL6 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --expected-lifetime=8h --memory=4GB --role=Analysis file://${pnfsOutDir}/runRecoCoarseScanNoQJob${fileNum}.sh

  rm -f runRecoCoarseScanNoQJob${fileNum}.sh
