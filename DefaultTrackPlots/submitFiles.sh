...
for file in `ls SplitFileList*`; do 
  ...
  if [ -f ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScan.fcl ]; then
    rm -f ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScan.fcl  
  fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  gm2 -c RunRecoAndPlotVeryCoarseScan.fcl -s \$file -T recoPlots\${id}.root
