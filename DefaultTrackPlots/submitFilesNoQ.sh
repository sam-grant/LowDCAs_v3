...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/DefaultTrackPlots/NoQ
...
split MainFileList.txt -l 3 -a 3 -d SplitFileList
...
for file in `ls SplitFileList*`; do 
  ...
  if [ -f ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScanNoQ.fcl ]; then
    rm -f ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScanNoQ.fcl  
  fi
  ...
  ifdh cp RunRecoAndPlotVeryCoarseScanNoQ.fcl ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScanNoQ.fcl
  ...
  echo "Copied RunRecoAndPlotVeryCoarseScanNoQ.fcl, libgm2tracker_analyses_LowDCAsPlots_module.so, and xrootdFileList${fileNum} to ${pnfsOutDir}"
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScanNoQ.fcl\`" ]; then
  ifdh cp ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScanNoQ.fcl ./RunRecoAndPlotVeryCoarseScanNoQ.fcl
else 
  echo "${pnfsOutDir}/RunRecoAndPlotVeryCoarseScanNoQ.fcl does not exist"
fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  gm2 -c RunRecoAndPlotVeryCoarseScanNoQ.fcl -s \$file -T recoPlots\${id}.root
