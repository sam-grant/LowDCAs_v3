...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsRecoScan_15921/Q
...
for file in `ls SplitFileList*`; do 
  ...
  ifdh cp RunRecoAndPlotCoarseScan.fcl ${pnfsOutDir}/RunRecoAndPlotCoarseScan.fcl
  ...
  echo "Copied RunRecoAndPlotCoarseScan.fcl, libgm2tracker_analyses_LowDCAsPlots_module.so, and xrootdFileList${fileNum} to ${pnfsOutDir}"
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/RunRecoAndPlotCoarseScan.fcl\`" ]; then
  ifdh cp ${pnfsOutDir}/RunRecoAndPlotCoarseScan.fcl ./RunRecoAndPlotCoarseScan.fcl
else 
  echo "${pnfsOutDir}/RunRecoAndPlotCoarseScan.fcl does not exist"
fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  gm2 -c RunRecoAndPlotCoarseScan.fcl -s \$file -T recoCoarseScanPlots\${id}.root
