...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/DefaultTrackPlots/Q
...
for file in `ls SplitFileList*`; do 
  ...
  for line in `cat $file`; do
    ...
    if [ -f ${pnfsOutDir}/recoCoarseScanPlots${id}.root ]; then
      echo "${pnfsOutDir}/recoCoarseScanPlots${id}.root already exists, skipping..."
      continue
    fi
  ...
  if [ -f ${pnfsOutDir}/RunRecoAndPlotCoarseScan.fcl ]; then
    rm -f ${pnfsOutDir}/RunRecoAndPlotCoarseScan.fcl  
  fi
  ...
  ifdh cp RunRecoAndPlotVeryCoarseScan.fcl ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScan.fcl
  ...
  echo "Copied RunRecoAndPlotVeryCoarseScan.fcl, libgm2tracker_analyses_LowDCAsPlots_module.so, and xrootdFileList${fileNum} to ${pnfsOutDir}"
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScan.fcl\`" ]; then
  ifdh cp ${pnfsOutDir}/RunRecoAndPlotVeryCoarseScan.fcl ./RunRecoAndPlotVeryCoarseScan.fcl
else 
  echo "${pnfsOutDir}/RunRecoAndPlotVeryCoarseScan.fcl does not exist"
fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  gm2 -c RunRecoAndPlotVeryCoarseScan.fcl -s \$file -T recoCoarseScanPlots\${id}.root

  # Only copy back if job completed successfully (output from last command was 0)
  if [ \$? -eq 0 ]; then
    if [ -f recoCoarseScanPlots\${id}.root ]; then
      ifdh mv recoCoarseScanPlots\${id}.root ${pnfsOutDir}/recoCoarseScanPlots\${id}.root
    else 
      echo "recoCoarseScanPlots\${id}.root does not exist.  ls reads:"
      ls
    fi
  fi
done
EOF
  ...
  if [ -f ${pnfsOutDir}/runRecoCoarseScanJob${fileNum}.sh ]; then
    rm -f ${pnfsOutDir}/runRecoCoarseScanJob${fileNum}.sh
  fi
    ifdh cp runRecoCoarseScanJob${fileNum}.sh ${pnfsOutDir}/runRecoCoarseScanJob${fileNum}.sh
  while [ ! -f ${pnfsOutDir}/runRecoCoarseScanJob${fileNum}.sh ]; do
    echo "${pnfsOutDir}/runRecoCoarseScanJob${fileNum}.sh not found after ifdh cp.  Sleeping 5..."
    sleep 5
  done

  #Submit grid job
  jobsub_submit -N 1 -G gm2 --OS=SL6 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --expected-lifetime=8h --memory=4GB --role=Analysis file://${pnfsOutDir}/runRecoCoarseScanJob${fileNum}.sh
