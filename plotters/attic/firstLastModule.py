...
filters1 = ["oneHitAllDCAs", "oneHitLowDCAs", "oneHitHighDCAs"]
filters2 = ["firstOrLastModuleAllDCAs", "firstOrLastModuleLowDCAs", "firstOrLastModuleHighDCAs" ]
...
for i in range(len(fitMode)):
	...
	for j in range(len(config)):
		...
		for k in range(len(filters1)):
			...
			# Get regular file
			file1 = TFile.Open("../runSimPlots_v2/"+fitMode[i]+"/"+config[j]+"/filteredPlots_"+filters1[k]+".root")
			file2 = TFile.Open("../runSimPlots_v2/"+fitMode[i]+"/"+config[j]+"/filteredPlots_"+filters2[k]+".root")
			print(file1)
			print(file2)

			# Grab all hits file
			h1 = file1.Get("oneWrong/Run");
			h2 = file2.Get("oneWrong/Run");
			print(h1)
			print(h1.GetEntries())
			print(h2)
			print(h2.GetEntries())
		...
		tgr1.SetMinimum(0)
		DrawScatOverlay(tgr1,tgr2,"Any module","First/last module",";Filter condition on wrong hits;Tracks with one wrong hit", "../images/"+fitMode[i]+"/"+config[j]+"/firstLastModule")
