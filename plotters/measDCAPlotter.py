...
filters = ["notFirstOrLastModuleAllDCAs", "notFirstOrLastModuleHighDCAs", "notFirstOrLastModuleLowDCAs",
				"firstOrLastModuleAllDCAs", "firstOrLastModuleHighDCAs", "firstOrLastModuleLowDCAs",
 				...
 				"oneHitInViewAllDCAs", "oneHitInViewHighDCAs", "oneHitInViewLowDCAs", "oneHitAll"]
...
for i in range(len(fitMode)):
	...
	for j in range(len(config)):
		...
		# Draw ratio
		FancyDraw1D(ratio, ";Measured DCA [#mum];Fraction of wrong LR hits","../images/"+fitMode[i]+"/"+config[j]+"/fracWrongMeasDCAsOneWrongZoom")
