...
pnfsInDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsNominalSim/NoQ

#Make sure file list exists
if [ ! -f ../commoncd .txt ] || [ `cat ../commonFilesForHadd.txt | wc -l` -eq 0 ]; then
...
for file in `ls FileList_*.txt`; do run=${file##*FileList_}; run=${run%%.*}; echo $run; done | xargs -i --max-procs=$nCores bash -c ". ../haddFileListNoQ.sh {}"

echo "Performing final hadd"

hadd -f nominalSimPlotsNoQ.root */nominalSimPlotsNoQ_*.root
