...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsScans/FineNoneWrong
...
for file in `ls SplitFileList*`; do 
  ...
  for line in `cat $file`; do
    ...
    if [ -f ${pnfsOutDir}/${fileNum}/simScanFineNoneWrong_${id}.root ]; then
      echo "${pnfsOutDir}/${fileNum}/simScanFineNoneWrong_${id}.root already exists, skipping..."
      continue
    fi
  ...
  if [ -f ${pnfsOutDir}/${fileNum}/RunSimAndPlotNominalScanFineNoneWrong.fcl ]; then
    rm -f ${pnfsOutDir}/${fileNum}/RunSimAndPlotNominalScanFineNoneWrong.fcl  
  fi
  ...
  ifdh cp RunSimAndPlotNominalScanFineNoneWrong.fcl ${pnfsOutDir}/${fileNum}/RunSimAndPlotNominalScanFineNoneWrong.fcl
  ...
  echo "Copied RunSimAndPlotNominalScanFineNoneWrong.fcl, libgm2tracker_analyses_LowDCAsTruthPlots_module.so, and xrootdFileList${fileNum} to ${pnfsOutDir}/${fileNum}"


# Make script that we'll want to run on the grid
cat <<EOF > runFineScanNoneWrongJob${fileNum}.sh
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/${fileNum}/RunSimAndPlotNominalScanFineNoneWrong.fcl\`" ]; then
  ifdh cp ${pnfsOutDir}/${fileNum}/RunSimAndPlotNominalScanFineNoneWrong.fcl ./RunSimAndPlotNominalScanFineNoneWrong.fcl
else 
  echo "${pnfsOutDir}/${fileNum}/RunSimAndPlotNominalScanFineNoneWrong.fcl does not exist"
fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  gm2 -c RunSimAndPlotNominalScanFineNoneWrong.fcl -s \$file -T simScanFineNoneWrong_\${id}.root

  # Only copy back if job completed successfully (output from last command was 0)
  if [ \$? -eq 0 ]; then
    if [ -f simScanFineNoneWrong_\${id}.root ]; then
      ifdh mv simScanFineNoneWrong_\${id}.root ${pnfsOutDir}/${fileNum}/simScanFineNoneWrong_\${id}.root
    else 
      echo "simScanFineNoneWrong_\${id}.root does not exist.  ls reads:"
      ls
    fi
  fi
done
EOF
  ...
  if [ -f ${pnfsOutDir}/${fileNum}/runFineScanNoneWrongJob${fileNum}.sh ]; then
    rm -f ${pnfsOutDir}/${fileNum}/runFineScanNoneWrongJob${fileNum}.sh
  fi
    ifdh cp runFineScanNoneWrongJob${fileNum}.sh ${pnfsOutDir}/${fileNum}/runFineScanNoneWrongJob${fileNum}.sh
  while [ ! -f ${pnfsOutDir}/${fileNum}/runFineScanNoneWrongJob${fileNum}.sh ]; do
    echo "${pnfsOutDir}/${fileNum}/runFineScanNoneWrongJob${fileNum}.sh not found after ifdh cp.  Sleeping 5..."
    sleep 5
  done

  #Submit grid job
  jobsub_submit -N 1 -G gm2 --OS=SL6 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --expected-lifetime=4h --memory=3GB --role=Analysis file://${pnfsOutDir}/${fileNum}/runFineScanNoneWrongJob${fileNum}.sh
  rm -f runFineScanNoneWrongJob${fileNum}.sh
