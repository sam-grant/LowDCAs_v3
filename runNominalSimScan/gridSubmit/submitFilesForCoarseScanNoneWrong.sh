...
pnfsOutDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsScans2/CoarseNoneWrong
...
for file in `ls SplitFileList*`; do 
  ...
  for line in `cat $file`; do
    ...
    if [ -f ${pnfsOutDir}/simScanCoarseNoneWrong_${id}.root ]; then
      echo "${pnfsOutDir}/simScanCoarseNoneWrong_${id}.root already exists, skipping..."
      continue
    fi
  ...
  if [ -f ${pnfsOutDir}/RunSimAndPlotNominalScanCoarseNoneWrong.fcl ]; then
    rm -f ${pnfsOutDir}/RunSimAndPlotNominalScanCoarseNoneWrong.fcl  
  fi
  ...
  ifdh cp RunSimAndPlotNominalScanCoarseNoneWrong.fcl ${pnfsOutDir}/RunSimAndPlotNominalScanCoarseNoneWrong.fcl
  ...
  echo "Copied RunSimAndPlotNominalScanCoarseNoneWrong.fcl, libgm2tracker_analyses_LowDCAsTruthPlots_module.so, and xrootdFileList${fileNum} to ${pnfsOutDir}"
...
if [ ! -z "\`ifdh ls ${pnfsOutDir}/RunSimAndPlotNominalScanCoarseNoneWrong.fcl\`" ]; then
  ifdh cp ${pnfsOutDir}/RunSimAndPlotNominalScanCoarseNoneWrong.fcl ./RunSimAndPlotNominalScanCoarseNoneWrong.fcl
else 
  echo "${pnfsOutDir}/RunSimAndPlotNominalScanCoarseNoneWrong.fcl does not exist"
fi
...
for file in \`cat xrootdFileList${fileNum}.txt\`; do
  ...
  if [ \$? -eq 0 ]; then
    if [ -f simScanCoarseNoneWrong_\${id}.root ]; then
      ifdh mv simScanCoarseNoneWrong_\${id}.root ${pnfsOutDir}/simScanCoarseNoneWrong_\${id}.root
    else 
...
EOF
  ...
  if [ -f ${pnfsOutDir}/runCoarseScanNoneWrongJob${fileNum}.sh ]; then
    rm -f ${pnfsOutDir}/runCoarseScanNoneWrongJob${fileNum}.sh
  fi
    ifdh cp runCoarseScanNoneWrongJob${fileNum}.sh ${pnfsOutDir}/runCoarseScanNoneWrongJob${fileNum}.sh
  while [ ! -f ${pnfsOutDir}/runCoarseScanNoneWrongJob${fileNum}.sh ]; do
    echo "${pnfsOutDir}/runCoarseScanNoneWrongJob${fileNum}.sh not found after ifdh cp.  Sleeping 5..."
    sleep 5
  done

  #Submit grid job
  jobsub_submit -N 1 -G gm2 --OS=SL6 --resource-provides=usage_model=DEDICATED,OPPORTUNISTIC --expected-lifetime=6h --memory=3GB --role=Analysis file://${pnfsOutDir}/runCoarseScanNoneWrongJob${fileNum}.sh
