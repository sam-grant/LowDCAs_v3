...
#   return
# fi
mkdir coarseNoneWrong
cd coarseNoneWrong
pwd

pnfsInDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsScans/CoarseNoneWrong
...
for file in `ls FileList_*.txt`; do run=${file##*FileList_}; run=${run%%.*}; echo $run; done | xargs -i --max-procs=$nCores bash -c ". ../haddFileListCoarseNoneWrong.sh {}"

echo "Performing final hadd"

hadd -f nominalSimScanPlotsCoarseNoneWrong.root */nominalSimScanPlotsCoarseNoneWrong_*.root
