...
for file in `ls FileList_*.txt`; do run=${file##*FileList_}; run=${run%%.*}; echo $run; done | xargs -i --max-procs=$nCores bash -c ". ../haddFileListFineNoneWrong.sh {}"

echo "Performing final hadd"

hadd -f nominalSimScanPlotsFineNoneWrong.root */nominalSimScanPlotsFineNoneWrong_*.root
