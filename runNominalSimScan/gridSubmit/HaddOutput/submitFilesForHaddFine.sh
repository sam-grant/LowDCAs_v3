...
pnfsInDir=/pnfs/GM2/scratch/users/${USER}/LowDCAsScans/Fine
...
for file in `ls FileList_*.txt`; do run=${file##*FileList_}; run=${run%%.*}; echo $run; done | xargs -i --max-procs=$nCores bash -c ". ../haddFileListFine.sh {}"

echo "Performing final hadd"

hadd -f nominalSimScanPlotsFine.root */nominalSimScanPlotsFine_*.root
